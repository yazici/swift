...
function(_add_swift_library_single target name)
  ...
  if(SWIFTLIB_SINGLE_TARGET_LIBRARY)
    set(uc_include_path "${SWIFT_${SWIFTLIB_SINGLE_SDK}_${SWIFTLIB_SINGLE_ARCHITECTURE}_ICU_UC_INCLUDE}")
    set(i18n_include_path "${SWIFT_${SWIFTLIB_SINGLE_SDK}_${SWIFTLIB_SINGLE_ARCHITECTURE}_ICU_I18N_INCLUDE}")
    if("${SWIFTLIB_SINGLE_SDK}" STREQUAL "FUCHSIA")
      # Fuchsia only has one shared ICU include
      set(uc_include_path "${SWIFT_FUCHSIA_ICU_UC_INCLUDE}")
      set(i18n_include_path "${SWIFT_FUCHSIA_ICU_I18N_INCLUDE}")
    endif() 
    if(NOT "${uc_include_path}" STREQUAL "" AND
       NOT "${uc_include_path}" STREQUAL "/usr/include" AND
       NOT "${uc_include_path}" STREQUAL "/usr/${SWIFT_SDK_${SWIFTLIB_SINGLE_SDK}_ARCH_${SWIFTLIB_SINGLE_ARCHITECTURE}_TRIPLE}/include" AND
       NOT "${uc_include_path}" STREQUAL "/usr/${SWIFT_SDK_${SWIFT_HOST_VARIANT_SDK}_ARCH_${SWIFT_HOST_VARIANT_ARCH}_TRIPLE}/include")
     target_include_directories("${target}" SYSTEM PRIVATE "${uc_include_path}")
    endif()
    if(NOT "${i18n_include_path}" STREQUAL "" AND
       NOT "${i18n_include_path}" STREQUAL "/usr/include" AND
       NOT "${i18n_include_path}" STREQUAL "/usr/${SWIFT_SDK_${SWIFTLIB_SINGLE_SDK}_ARCH_${SWIFTLIB_SINGLE_ARCHITECTURE}_TRIPLE}/include" AND
       NOT "${i18n_include_path}" STREQUAL "/usr/${SWIFT_SDK_${SWIFT_HOST_VARIANT_SDK}_ARCH_${SWIFT_HOST_VARIANT_ARCH}_TRIPLE}/include")
     target_include_directories("${target}" SYSTEM PRIVATE "${i18n_include_path}")
    endif()
  endif()
